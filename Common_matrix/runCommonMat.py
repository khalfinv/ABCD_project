...
def analyzeCorrelationsAndPlotConnectome(matrix, coords, networks, out_folder):
	colors = []
	patches_list = []
	index2network = {}
	min_r = 0.7
	...
	correlated_coords_file = open(out_folder + "/correlated_coords_" + str(networks) + ".txt", "w")
	for item in correlated_coords.items():
		correlated_coords_file.write(str(item)+ "\n")
	correlated_coords_file.close()
	plot_name = out_folder + "/brain_plot" + str(networks) + ".png"
	if(len(correlated_coords) > 0):
		plotConnectome(matrix, coords, plot_name, colors, patches_list, min_r)
	
def plotConnectome(matrix, coords, plot_path, colors, patches_list, min_r):
	fig = plt.figure()
...
def runAllCombinations(common_cov_mat, common_cor_mat, out_folder, coords):
    ...
    for pair in itertools.combinations(networkToIndexDic.dic.keys(), r=2):
        ...
        analyzeCorrelationsAndPlotConnectome(common_cor_mat_sliced, coords_sliced, pair, out_folder)

				   
def runSomeNetworks(common_cov_mat,common_cor_mat,out_folder, networks , coords):
...
if __name__ == "__main__":
	...
	if (args.networks != None):
		if(args.networks == ["all"]):
			runAllCombinations(common_cov_mat, common_cor_mat, args.out_folder, coords) 
		else:
			runSomeNetworks(common_cov_mat, common_cor_mat,args.out_folder,args.networks, coords)
