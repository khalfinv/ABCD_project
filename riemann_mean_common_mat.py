...
def createCommonMat(subjects_data_dict, exclusion_criteria):
    ...
    riemann_mean_cov_mat = riemann_mean = mean_covariance(np.array(covars), metric='riemann', sample_weight=None)
...
def runAllCombinations(common_cov_mat, common_cor_mat, out_folder):
    ...
    for pair in itertools.combinations(networkToIndexDic.dic.keys(), r=2):
        ...
        plotMatrix(common_cor_mat_sliced, out_folder + "/riemann_mean_cor_matrix_" + network1 + "_" + network2 + ".png", [network1, network2], "Riemann mean correlation matrix", ticks, -1., 1.)
        plotMatrix(common_cov_mat_sliced, out_folder + "/riemann_mean_cov_matrix_" + network1 + "_" + network2 + ".png", [network1, network2], "Riemann mean covariance matrix",
...
def runSomeNetworks(common_cov_mat,common_cor_mat,out_folder, networks ):
    ...
    common_cov_mat_sliced = common_cov_mat[listToSlice, :][:, listToSlice] 
    common_cor_mat_sliced = common_cor_mat[listToSlice, :][:, listToSlice] 
    plotMatrix(common_cor_mat_sliced, out_folder + "/riemann_mean_cor_matrix_" + str(networks) + ".png", networks, "Riemann mean correlation matrix", ticks, -1., 1.)
    plotMatrix(common_cov_mat_sliced, out_folder + "/riemann_mean_cor_matrix_" + str(networks) + ".png", networks, "Riemann mean covariance matrix", ticks,
...
if __name__ == "__main__":
    ...
    if args.use_prev is False:
        ...
        f = open(args.out_folder + '/RiemannMeanCovAndCorMat.pkl', mode="wb")
    ...
    plotMatrix(common_cor_mat, args.out_folder + "/riemann_mean_cor_matrix.png",networkToIndexDic.dic.keys(), "Riemann mean correlation matrix", ticks, -1., 1.)
    plotMatrix(common_cov_mat, args.out_folder + "/riemann_mean_cov_matrix.png",networkToIndexDic.dic.keys(), "Riemann mean covariance matrix",
